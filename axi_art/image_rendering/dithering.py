...
STUCKI = (np.array([[0, 0, 0, 8, 4],
                    [2, 4, 8, 4, 2],
                    [1, 2, 4, 2, 1]]),)
SIERRA = (np.array([[0, 0, 0, 5, 3],
                    [2, 4, 5, 4, 2],
                    [0, 2, 3, 2, 0]]),)
ATKINSON = (np.array([[0, 0, 0, 1, 1],
                      [0, 1, 1, 1, 0],
                      [0, 0, 1, 0, 0]]), 8)
BURKES = (np.array([[0, 0, 0, 8, 4],
                    [2, 4, 8, 4, 2]]),)
    ...
    for y in range(out.shape[0]):
        for x in range(out.shape[1]):
            ...
            if x - diffusion.shape[1] // 2 < 0:
                ...
                local_diff = local_diff[:, :-(diffusion.shape[1] // 2 - (out.shape[1] - x - 1))]
            if y + diffusion.shape[0] - 1 >= out.shape[0]:
                local_diff = local_diff[:-(diffusion.shape[0] - (out.shape[0] - y)), :]
            out_slice = out[y:y + local_diff.shape[0],
                        max(0, x - diffusion.shape[1] // 2): min(out.shape[1], x + diffusion.shape[1] // 2 + 1)]
